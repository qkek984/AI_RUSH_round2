...
def evaluate(prediction, ground_truth):
    if set(prediction['image_name']) != set(ground_truth['image_name']):
    ...
    df = prediction.merge(ground_truth, on='image_name')
    f1_array = f1_score(df['answer'], df['y_pred'], average=None)
    score = gmean(f1_array)
    return score
